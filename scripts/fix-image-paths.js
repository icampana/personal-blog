...
function processFile(filePath) {
  try {
    ...
    const relativeImagePattern = /!\[([^\]]*)\]\((?!http|\/|#)([^)]+\.(jpg|jpeg|png|gif|webp|svg))\)/gi;

    // Fix relative imports starting with ./
    newContent = newContent.replace(imageImportPattern, (match, alt, imagePath) => {
    ...
    newContent = newContent.replace(relativeImagePattern, (match, alt, imagePath, ext) => {
    ...
    newContent = newContent.replace(htmlImgPattern, (match, beforeSrc, imagePath, afterSrc) => {
    ...
    const htmlImgRelativePattern = /<img([^>]*)\s+src=["'](?!http|\/|#)([^"']+\.(jpg|jpeg|png|gif|webp|svg))["']([^>]*)>/gi;
    newContent = newContent.replace(htmlImgRelativePattern, (match, beforeSrc, imagePath, ext, afterSrc) => {

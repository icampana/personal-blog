...
const ThemeToggle: React.FC = () => {
  ...
  useEffect(() => {
    ...
    let initialTheme = 'winter'; // default light theme
  ...
  if (!mounted) {
    return (
      <div className="w-12 h-6 bg-base-300 rounded-full animate-pulse"></div>
    );
  }
  ...
  return (
    <label className="swap swap-rotate text-base-content cursor-pointer" htmlFor="theme-toggle">
      ...
      {/* Sun icon */}
      <svg
        className={`swap-off fill-current w-6 h-6 transition-transform duration-300 ${
      ...
      {/* Moon icon */}
      <svg
        className={`swap-on fill-current w-6 h-6 transition-transform duration-300 ${

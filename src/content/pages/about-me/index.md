---
wordpress_id: 1061
path: /sobre-el-autor
author: ivan
comments: false
title: Sobre Mi
date: 2020-12-18T00:00:00.000Z
---

De Guayaquil a Porto: 20 años construyendo el futuro de la web 🚀

Empecé mi carrera profesional en 2003 cuando la web aún estaba definiendo su futuro. Desde entonces, he tenido el privilegio de trabajar en proyectos que impactan a millones de personas: desde medios digitales que generan 10+ millones de visitas mensuales, hasta plataformas fintech que empoderan asesores financieros con inteligencia artificial.

## 💼 HOY:

Lidero el desarrollo en [AssetLink AI](https://assetlink.ai/) (NY, EEUU), donde construimos una plataforma SaaS fintech certificada SOC2 que integra AI/LLM (OpenAI, Google Gemini) con Bases de datos Vectoriales y agentes personalizados. Conectamos a los agentes financieros, integrando herramientas como Microsoft Dynamics y Salesforce con herramientas de IA, creando una fuente única de verdad que mejora la toma de decisiones.

## 🌟 MI TRAYECTORIA:

• [G/O Media](https://g-omedia.com/): Fui parte del equipo de desarrollo y mantenimiento de marcas icónicas (Gizmodo, Kotaku, Lifehacker, The Onion) con arquitecturas de microservicios y millones de usuarios simultáneos

• [Tentrr (USA)](https://www.tentrr.com/): Lideré la transformación de monolito a serverless, diseñando arquitecturas modernas en AWS

• [BairesDev](https://www.bairesdev.com/), [WeKnow](https://weknowinc.com/): Desarrollé soluciones para clientes de USA implementando portales tanto comunicacionales como transaccionales, incluyendo empresas de gran envergadura.

• [DOMO (Co-fundador)](https://domo.ec/): Lideré equipo de 8 desarrolladores trabajando con grandes empresas ecuatorianas (Ecuavisa, El Universo, De Prati, Interagua)

## 🛠️ LO QUE HAGO:

Me especializo en arquitecturas escalables, código limpio y equipos de alto rendimiento. Trabajo con React, Node.js, Python, AWS y ahora AI/ML. He migrado sistemas legacy, optimizado plataformas de alto tráfico y liderado equipos distribuidos en 3 continentes.

## 👨‍🏫 COMPARTIENDO CONOCIMIENTO:

* Blog técnico desde 2004 (370+ artículos) sobre desarrollo, arquitectura, clean code y experiencias personales.
* Formador en bootcamps: Awana, Spotify Interview Preparation
* Ex-profesor universitario (Universidad Santa María - Campus Guayaquil, Casa Grande)
* Mentor en Techstars Startup Weekend y Lean Startup Machine
* Co-organizador del primer Drupal Camp en Ecuador

## 🌍 CIUDADANO DEL MUNDO:

Trabajo remotamente desde Porto con equipos en USA, Europa y LATAM. Hablo 4 idiomas (Español Nativo, Inglés C2, Portugués B2, Francés B2) y he colaborado con empresas desde Silicon Valley hasta Latinoamérica.

¿Mi próximo desafío? Ayudar a equipos y empresas a construir productos que realmente importen, usando las mejores prácticas, tecnologías modernas y un enfoque centrado en las personas.

## 👀 Curiosidades

Algunos highlights de mi historia personal:

* Ingeniero informático de Gestión por la [Universidad Técnica Federico Santa María](https://www.usm.cl/) (Campus Guayaquil) con un diplomado en Comunicación Aplicado a la Publicidad por la [Universidad Casa Grande](https://www.casagrande.edu.ec/).
* Más de 20 años dedicándome a desarrollar experiencias digitales para empresas, startups y trabajando con diferentes culturas (España, EEUU, Argentina, Chile, Ecuador).
* Alumni de la generación 15 de [Startup Chile](https://startupchile.org/) Seed.
* Trabajando de forma remota desde 2016.

📍 [Portafolio completo](/portafolio/)

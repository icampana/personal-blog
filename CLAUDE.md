...
This is an Astro-based blog with 370+ posts spanning 20+ years (2004-2025), migrated from Next.js. The site uses TypeScript, Tailwind CSS with DaisyUI, and includes Keystatic for visual content editing.

## Development Commands

### Development
```bash
# Start dev server (admin at /keystatic, runs on localhost:4321)
pnpm run dev
...
### Building
```bash
# Production build (includes prebuild checks, search index, and verification)
pnpm run build
...
Four content types defined in `src/content.config.ts`:
...
- **Videos** (`src/content/videos/`): YouTube videos with videoId, date, description.

All content uses Zod schemas for validation. Each type is organized into per-locale directories (`{es,en,pt,fr}/<slug>.md`); Spanish (`es/`) is the source of truth and translations are generated. Path resolution: posts use `/posts/{path || slug}`, pages use `/{path || slug}`.

### Keystatic Integration

Keystatic (`@keystatic/astro` + `@keystatic/core`) runs in local mode and provides the visual content editor at `/keystatic` (dev only). Configured in `keystatic.config.ts`:

- **Spanish-only editing**: collections point at the `es/` dirs (e.g. `src/content/posts/es/*`). Translations are generated by the translate script, not edited in the CMS.
- **Body field**: `fields.markdoc` with `extension: 'md'`, writing plain `.md` files.
- **Media**: images upload to `public/photos/` and are referenced as `/photos/<file>`.
- **Dev gating**: the Keystatic Astro integration is only active in dev (`NODE_ENV !== 'production'`); production builds ship no admin surface.
...
- **Node**: v20+, pnpm v9+
- **Build command**: `pnpm run build:production` (lints, tests, generates search index, builds Astro, verifies output)
- **Environments**:
...
- Content is edited in Spanish via Keystatic (`/keystatic`, dev only); translations are generated by `pnpm run translate` into `en|pt|fr/` dirs
...
- **Symptom**: `/keystatic` shows 404 or does not render
- **Cause**: The Keystatic Astro integration is dev-only (`NODE_ENV === 'production'` gates it out); or a `src/pages/keystatic/[...route].astro` file conflicts with the integration
- **Solution**: Run `pnpm run dev` (plain `astro dev`); do not create a manual keystatic route page

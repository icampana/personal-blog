...
export async function getStaticPaths() {
	...
	const paths: string[] = Array.from(
		...
		(_value, index) => `/posts/page/${index + 1}`,
	);
...
const Home: NextPage<{ posts: Post[]; currentPage: number }> = (props) => {
	...
	return (
		<div className="container mx-auto">
			<Head>
				<title>
					Listado de Posts - Página {currentPage} | {site.title}
				</title>
					...
					<div>
						{currentPage > 1 && (
							<Link href={`/posts/page/${currentPage - 1}`}>
								<span className="block float-left py-5 font-bold text-xl text-red-700">
									← Anterior
								</span>
							</Link>
						)}
						...
						<Link href={`/posts/page/${currentPage + 1}`}>
							<span className="block float-right py-5 font-bold text-xl text-red-700">
								Siguiente →
							</span>
						</Link>

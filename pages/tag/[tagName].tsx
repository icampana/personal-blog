...
import { getPostsListing, type PostFilter } from "components/utils/posts";
...
export async function getStaticPaths() {
	const tagsList: Array<string> = allPosts.reduce(
...
export async function getStaticProps(context: GetStaticPropsContext) {
	...
	const filterByTag: PostFilter = (post: Post) =>
		getCleanTags(post).includes(tagName);
...
const TagPage: NextPage<{ posts: Post[]; currentTag: string }> = (props) => {
	...
	return (
		<div className="container mx-auto">
			<Head>
				<title>
					Listado de publicaciones sobre: {tagName} | {site.title}
				</title>
			...
			<main className="max-w-6xl mx-auto">
				<Header>
					<h1 className="font-sans font-bold text-2xl capitalize">
						Publicaciones sobre <em>{tagName}</em>
					</h1>
				</Header>

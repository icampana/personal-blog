...
- **Keystatic:** Local-mode content editor at `/keystatic` (dev only), configured in `keystatic.config.ts`. Content lives in per-locale dirs `src/content/{posts,pages,projects,videos}/{es,en,pt,fr}/`; Spanish (`es/`) is the source of truth and translations are generated by script.
